  ...
  */
  ...
  if (!(thd->variables.sql_mode & MODE_NO_TABLE_OPTIONS) && !foreign_db_mode)
  {
    ...
    */
    ...
    if (share->db_create_options & HA_OPTION_PERSISTENT_STATS)
      packet->append(STRING_WITH_LEN(" PERSISTENT_STATS=1"));
    if (share->db_create_options & HA_OPTION_NO_PERSISTENT_STATS)
      packet->append(STRING_WITH_LEN(" PERSISTENT_STATS=0"));
				    ...
				    LEX_STRING *table_name)
{
  ...
  else
  {
    ...
    if (share->db_create_options & HA_OPTION_PERSISTENT_STATS)
      ptr=strmov(ptr," persistent_stats=1");

    if (share->db_create_options & HA_OPTION_NO_PERSISTENT_STATS)
      ptr=strmov(ptr," persistent_stats=0");
