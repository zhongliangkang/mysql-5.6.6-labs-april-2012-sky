...
/* See my_hash_sort_simple() for usage */
#define	MY_HASH_ADD(N1, N2, V) \
  do { \
    (N1)^= (ulong) ((((uint) (N1) & 63)+(N2)) * ((uint) V)) + ((N1) << 8); \
    (N2)+= 3; \
  } while(0)

#define MY_SORT_LEN_GT_0(K, LEN, NR1, NR2) \
  do { \
    ulong T123= *NR1; \
    ulong U123= *NR2; \
    register size_t N123= (LEN + 3) / 4; \
    DBUG_ASSERT(LEN > 0); \
    \
    switch (LEN % 4)  { \
    case 0: do {  MY_HASH_ADD(T123, U123, MY_SORT_ORDER(*K++)); \
    case 3:       MY_HASH_ADD(T123, U123, MY_SORT_ORDER(*K++)); \
    case 2:       MY_HASH_ADD(T123, U123, MY_SORT_ORDER(*K++)); \
    case 1:       MY_HASH_ADD(T123, U123, MY_SORT_ORDER(*K++)); \
                } while (--N123 > 0); \
    } \
    \
    *NR1= T123; \
