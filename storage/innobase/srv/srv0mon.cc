					...
					counter */
{
	...
	case MONITOR_OVLD_BUF_POOL_READS:
		value = srv_counter.buf_pool_reads;
		break;
	...
	case MONITOR_OVLD_BUF_POOL_WRITE_REQUEST:
		value = srv_counter.buf_pool_write_requests;
		break;

	/* innodb_buffer_pool_wait_free */
	case MONITOR_OVLD_BUF_POOL_WAIT_FREE:
		value = srv_counter.buf_pool_wait_free;
		break;
	...
	case MONITOR_OVLD_OS_LOG_WRITTEN:
		value = (mon_type_t) srv_counter.os_log_written;
		break;
	...
	case MONITOR_OVLD_OS_LOG_PENDING_WRITES:
		value = srv_counter.os_log_pending_writes;
		update_min = TRUE;
		break;

	/* innodb_log_waits */
	case MONITOR_OVLD_LOG_WAITS:
		value = srv_counter.log_waits;
		break;

	/* innodb_log_write_requests */
	case MONITOR_OVLD_LOG_WRITE_REQUEST:
		value = srv_counter.log_write_requests;
		break;

	/* innodb_log_writes */
	case MONITOR_OVLD_LOG_WRITES:
		value = srv_counter.log_writes;
		break;

	/* innodb_dblwr_writes */
	case MONITOR_OVLD_SRV_DBLWR_WRITES:
		value = srv_counter.dblwr_writes;
		break;

	/* innodb_dblwr_pages_written */
	case MONITOR_OVLD_SRV_DBLWR_PAGES_WRITTEN:
		value = srv_counter.dblwr_pages_written;
		break;
	...
	case MONITOR_OLVD_ROW_READ:
		value = srv_counter.n_rows_read;
		break;

	/* innodb_rows_inserted */
	case MONITOR_OLVD_ROW_INSERTED:
		value = srv_counter.n_rows_inserted;
		break;

	/* innodb_rows_deleted */
	case MONITOR_OLVD_ROW_DELETED:
		value = srv_counter.n_rows_deleted;
		break;

	/* innodb_rows_updated */
	case MONITOR_OLVD_ROW_UPDTATED:
		value = srv_counter.n_rows_updated;
		break;

	/* innodb_row_lock_current_waits */
	case MONITOR_OVLD_ROW_LOCK_CURRENT_WAIT:
		value = srv_counter.n_lock_wait_current_count;
		break;

	/* innodb_row_lock_time */
	case MONITOR_OVLD_LOCK_WAIT_TIME:
		value = srv_counter.n_lock_wait_time / 1000;
		break;
	...
	case MONITOR_OVLD_ROW_LOCK_WAIT:
		value = srv_counter.n_lock_wait_count;
		break;
