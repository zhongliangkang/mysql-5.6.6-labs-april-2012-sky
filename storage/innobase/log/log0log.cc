...
part_loop:
	...
	srv_counter.log_write_requests.inc(0, 1);
}
					...
					lsn */
{
	...
	if (log_do_write) {
		...
		srv_counter.os_log_pending_writes.inc(0, 1);
		...
		srv_counter.os_log_pending_writes.dec();
	}
}
...
loop:
	...
	if ((next_offset % group->file_size == LOG_FILE_HDR_SIZE)
		...
		log_group_file_header_flush(group, (ulint)
					    ...
					    start_lsn);
		srv_counter.os_log_written.inc(0, OS_FILE_LOG_BLOCK_SIZE);

		srv_counter.log_writes.inc(0, 1);
	}
	...
	if (log_do_write) {
		...
		srv_counter.os_log_pending_writes.dec(0, 1);

		srv_counter.os_log_written.inc(0, write_len);
		srv_counter.log_writes.inc(0, 1);
	}
