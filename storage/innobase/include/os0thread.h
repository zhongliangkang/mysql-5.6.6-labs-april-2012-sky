...
template <typename Type, int N=8>
class ib_counter_t {
public:
	...
	* @param n  - is the amount to increment */
	void inc(Type n) UNIV_NOTHROW {
		m_counter[offset(get_index())] += n;
	}
	...
	void inc(size_t i, Type n) UNIV_NOTHROW {
		m_counter[offset(i)] += n;
	}
	...
	* @param - n is the amount to decrement */
	...
	void dec(size_t i, Type n) UNIV_NOTHROW {
		m_counter[offset(i)] -= n;
	}
...
private:
        ...
        size_t offset(size_t i) const UNIV_NOTHROW {
                return(((i % N) + 1) * (CACHE_LINE_SIZE / sizeof(Type)));
        }

	/* @return a random number, currently we use the thread id. Where it
	is a pointer, it may not work as effectively. */
	size_t get_index() const UNIV_NOTHROW {
