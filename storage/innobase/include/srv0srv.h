...
struct srv_counter_t {
	typedef ib_counter_t<lsn_t, IB_N_SLOTS> lsn_ctr_t;
	typedef ib_counter_t<ulint, IB_N_SLOTS> ulint_ctr_t;
	typedef ib_counter_t<ib_int64_t, IB_N_SLOTS> ib_int64_ctr_t;

	/** Count the amount of data written in total (in bytes) */
	ulint_ctr_t	data_written;

	/** Number of the log write requests done */
	ulint_ctr_t	log_write_requests;

	/** Number of physical writes to the log performed */
	ulint_ctr_t	log_writes;

	/** Amount of data written to the log files in bytes */
	lsn_ctr_t	os_log_written;

	/** Number of writes being done to the log files */
	ulint_ctr_t	os_log_pending_writes;

	/** We increase this counter, when we don't have enough
	space in the log buffer and have to flush it */
	ulint_ctr_t	log_waits;

	/** Count the number of times the doublewrite buffer was flushed */
	ulint_ctr_t	dblwr_writes;

	/** Store the number of pages that have been flushed to the
	doublewrite buffer */
	ulint_ctr_t	dblwr_pages_written;

	/** Store the number of write requests issued */
	ulint_ctr_t	buf_pool_write_requests;
	...
	ulint_ctr_t	buf_pool_wait_free;

	/** Count the number of pages that were written from buffer
	pool to the disk */
	ulint_ctr_t	buf_pool_flushed;

	/** Number of buffer pool reads that led to the reading of
	a disk page */
	ulint_ctr_t	buf_pool_reads;

	/** Number of data read in total (in bytes) */
	ulint_ctr_t	data_read;
	...
	ib_int64_ctr_t	n_lock_wait_time;

	/** Number of database lock waits */
	ulint_ctr_t	n_lock_wait_count;

	/** Number of threads currently waiting on database locks */
	ulint_ctr_t	n_lock_wait_current_count;
};
...
/** Global counters */
extern srv_counter_t	srv_counter;
