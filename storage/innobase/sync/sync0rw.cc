...
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_s_spin_wait_count;
/** number of spin loop rounds on rw-latches,
resulted during shared (read) locks */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_s_spin_round_count;

/** number of OS waits on rw-latches,
resulted during shared (read) locks */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_s_os_wait_count;

/** number of unlocks (that unlock shared locks),
set only when UNIV_SYNC_PERF_STAT is defined */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_s_exit_count;

/** number of spin waits on rw-latches,
resulted during exclusive (write) locks */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_x_spin_wait_count;
/** number of spin loop rounds on rw-latches,
resulted during exclusive (write) locks */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_x_spin_round_count;

/** number of OS waits on rw-latches,
resulted during exclusive (write) locks */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_x_os_wait_count;

/** number of unlocks (that unlock exclusive locks),
set only when UNIV_SYNC_PERF_STAT is defined */
UNIV_INTERN ib_counter_t<ib_int64_t, IB_N_SLOTS>	rw_x_exit_count;
	...
	ulint		line)	/*!< in: line where requested */
{
	...
	rw_s_spin_wait_count.inc((lint) thread_id, 1);
lock_loop:
	...
	if (TRUE == rw_lock_s_lock_low(lock, pass, file_name, line)) {
		rw_s_spin_round_count.inc((lint) thread_id, i);
		...
		lock->count_os_wait++;
		rw_s_os_wait_count.inc((lint) thread_id, 1);
	...
	ulint		line)	/*!< in: line where requested */
{
	...
	while (lock->lock_word < 0) {
		...
		rw_x_spin_round_count.inc((lint) thread_id, i);
		...
		if (lock->lock_word < 0) {
			...
			/* these stats may not be accurate */
			lock->count_os_wait++;
			rw_x_os_wait_count.inc((lint) thread_id, 1);
...
lock_loop:
	...
	if (rw_lock_x_lock_low(lock, pass, file_name, line)) {
		...
		return;	/* Locking succeeded */
                ...
                if (!spinning) {
                        spinning = TRUE;
                        rw_x_spin_wait_count.inc((lint) thread_id, 1);
		}
