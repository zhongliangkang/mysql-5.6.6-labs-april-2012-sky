	...
	ulint		line)		/*!< in: line where requested */
{
	...
	mutex_spin_wait_count.inc((lint) thread_id, 1);
...
spin_loop:
	...
	mutex_spin_round_count.inc((lint) thread_id, i);
	...
	if (mutex_test_and_set(mutex) == 0) {
		...
		ut_d(mutex->thread_id = thread_id);
#ifdef UNIV_SYNC_DEBUG
		mutex_set_debug_info(mutex, file_name, line);
#endif
		return;
	...
	mutex_os_wait_count.inc((lint) thread_id, 1);
	...
	FILE*	file)		/*!< in: file where to print */
{
	...
	fprintf(file,
		...
		(ib_uint64_t) mutex_exit_count,
		(ib_uint64_t) rw_s_exit_count,
		(ib_uint64_t) rw_x_exit_count);
#endif
	...
	fprintf(file,
		...
		(ib_uint64_t) rw_s_spin_wait_count,
		(ib_uint64_t) rw_s_spin_round_count,
		(ib_uint64_t) rw_s_os_wait_count,
		(ib_uint64_t) rw_x_spin_wait_count,
		(ib_uint64_t) rw_x_spin_round_count,
		(ib_uint64_t) rw_x_os_wait_count);
