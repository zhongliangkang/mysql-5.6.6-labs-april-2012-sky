...
table_opened:
	...
	dict_stats_init(
		...
		table->s->db_create_options & HA_OPTION_NO_PERSISTENT_STATS,
	...
	enum ha_rkey_function find_flag)/*!< in: search flags from my_base.h */
{
	...
	case DB_SUCCESS:
		error = 0;
		table->status = 0;
		srv_counter.n_rows_read.inc((size_t) prebuilt->trx->id, 1);
		break;
	case DB_RECORD_NOT_FOUND:
					...
					create statement string */
{
	...
	dict_stats_set_persistent(
		innobase_table,
		create_info->table_options & HA_OPTION_PERSISTENT_STATS,
		create_info->table_options & HA_OPTION_NO_PERSISTENT_STATS);
	...
	uint		table_changes)
{
	...
	dict_stats_set_persistent(
		prebuilt->table,
		info->table_options & HA_OPTION_PERSISTENT_STATS,
		info->table_options & HA_OPTION_NO_PERSISTENT_STATS);
...
static MYSQL_SYSVAR_ULONG(max_io_capacity, srv_max_io_capacity,
  ...
  NULL, innodb_max_io_capacity_update, 2000, 100, ~0UL, 0);
