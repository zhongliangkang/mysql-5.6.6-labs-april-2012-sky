					...
					min_n), otherwise ignored */
{
	...
	srv_counter.buf_pool_flushed.inc(count, count);
	...
	ulint		page_count)	/*!< in: number of pages flushed */
{
	...
	srv_counter.buf_pool_flushed.inc(page_count, page_count);
...
static
ulint
af_get_pct_for_dirty(void)
/*======================*/
{
	ulint dirty_pct = buf_get_modified_ratio_pct();

	ut_ad(srv_max_dirty_pages_pct_lwm
...
/*********************************************************************//**
Calculates if flushing is required based on number of redo generation
rate.
...
/*====================================*/
{
	...
	n_pages = page_cleaner_do_flush_batch(
		n_pages, oldest_lsn + lsn_avg_rate);
